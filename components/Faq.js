    ...
    <main className="p-5 md:p-20 h-auto" id="faq">
      <ScrollAnimationWrapper>
        <motion.h3
          ...
          className="text-center text-2xl md:text-3xl pb-10 font-semibold text-black-600 mb-4"
        >
          Pertanyaan Yang Sering Ditanyakan{" "}
          <a className="text-maroon-500">(FAQ)</a>
        </motion.h3>
      </ScrollAnimationWrapper>
      ...
      <div className="max-w-screen-xl mx-auto flex flex-col md:flex-row justify-center items-start">
        ...
        >
          <Image
            ...
            width={400}
            height={400}
            className="object-cover w-full h-auto max-w-sm md:max-w-full"
          />
        </motion.div>
          ...
          <ul className="flex flex-col">
            {[
              ...
              >
                ...
                >
                  <span className="text-sm md:text-base">{item.question}</span> {/* Ukuran teks yang responsif */}
                  <svg
                    className={`fill-current text-purple-700 h-6 w-6 transform transition-transform duration-500 ${rotateIcon(
                      index
                    )}`}
                    viewBox="0 0 20 20"
                  >
                ...
                <div
                  ...
                  className="border-l-2 border-maroon-500 overflow-hidden duration-500 transition-all"
                >
                  <p className="p-3 text-gray-900 text-sm md:text-base">{item.answer}</p> {/* Ukuran teks responsif */}
                </div>
              </motion.li>
            ))}
          </ul>
